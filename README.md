...
客户端只提交意图，不提交分数、命中结果、位置、计时或胜负。互坑扫雷在终局前不发送对手雷区；遗忘的地雷在本人确认后连本人旧雷图也不再发送，并且终局仍不公开完整雷图。通用会话键 `two-player-games-session` 只保存房间码、席位、token、游戏 ID 和玩家名，绝不保存雷图。
...
3. 只通过显式按席位序列化器发送公开状态；隐藏信息不得进入广播、重连、DOM 或浏览器存储。
