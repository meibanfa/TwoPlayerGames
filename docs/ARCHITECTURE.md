...
定时炸弹扫雷是独立的 `timed-mines` 游戏，不是遗忘的地雷的模式条件。纯规则位于 `js/games/timed-mines-logic.js`，权威状态机位于 `server/games/timed-mines.js`。服务器分别保存双方普通雷、带编号定时炸弹、抽象启动状态、以对手已完成移动为单位的倒计时、全局移动数、结算格和公开爆炸历史。启动与倒计时状态只序列化席位、编号、状态和剩余次数；定时炸弹中心只在实际爆炸后成为公开事件。确认后本人和对手都收不到隐藏坐标，只有 `FINISHED` 附加完整原始爆炸物地图。倒计时由移动事务同步推进，不使用墙钟回调；重连保持权威计数，重开直接丢弃全部旧状态。

## 状态与清理

通用房间先处于 `WAITING`，第二人加入后由游戏处理器进入其布雷阶段。互坑扫雷为 `PLACING → SWEEPING → FINISHED`；遗忘的地雷与定时炸弹扫雷为 `PLACING → PLAYING ↔ REENTRY → FINISHED`。断线计时继续，房间在宽限期内保留；到期会通知仍在线玩家、清理全部计时器并删除房间。所有计时器回调都必须验证房间和状态仍是创建它们时的对象，避免旧回调污染重开或已删除房间。
