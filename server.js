...
wss.on("connection", (ws, req) => { const origin = req.headers.origin; if (origin && origin !== `http://${req.headers.host}` && origin !== `https://${req.headers.host}`) return ws.close(1008, "origin"); ws.clientIp = req.socket.remoteAddress || "unknown"; ws.on("message", (raw) => { let msg; try { msg = JSON.parse(raw); } catch { return sendError(ws, "消息格式无效。"); }
  ...
  if (msg.type === "join") { const room = rooms.get(String(msg.code || "")); if (!room || room.players[1] || room.tokens[1] || !room.players[0] || room.players[0] === ws) return sendError(ws, "房间不存在、已满或房主正在重连。"); removeMembership(ws); clearTimeout(room.waitingTimer); room.players[1] = ws; room.names[1] = cleanName(msg.playerName, "玩家 2"); room.tokens[1] = makeToken(); ws.roomCode = room.code; ws.seat = 1; room.state.placementDeadline = Date.now() + PLACEMENT_MS; room.players.forEach((p, seat) => send(p, "start", { code: room.code, gameId: "minesweeper-duel", seat, playerNames: room.names, phase: "PLACING", token: room.tokens[seat], placementDeadline: room.state.placementDeadline })); room.placementTimer = setTimeout(() => autoPlacement(room), PLACEMENT_MS); return; }
