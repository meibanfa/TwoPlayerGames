...
function handleAction(room, ws, msg) {
  ...
  if (!room.players[0] || !room.players[1]) return sendError(ws, "请等待好友加入。");
  ...
  if (action === "reveal") { if (p.revealed[seat].has(msg.cell) || p.flags[seat].has(msg.cell)) return sendError(ws, "请先取消旗子。"); const board = p.boards[seat]; if (board.has(msg.cell)) { p.revealed[seat].add(msg.cell); p.mineHits[seat].add(msg.cell); p.mistakes[seat]++; p.penalty[seat] += 10_000; send(ws, "revealResult", { cells: [{ cell: msg.cell, mine: true }], mistakes: p.mistakes[seat], penalty: p.penalty[seat] }); } else { const cells = Logic.floodReveal([...board], msg.cell, [...p.revealed[seat]]); cells.forEach((x) => p.revealed[seat].add(x.cell)); send(ws, "revealResult", { cells }); } emitProgress(room); finish(room, seat); return; }
  sendError(ws, "未知操作。");
}
function disconnect(ws) { const room = roomFor(ws); if (!room || room.players[ws.seat] !== ws) return; room.players[ws.seat] = null; ws.roomCode = null; send(room.players[1 - ws.seat], "opponentDisconnected", {}); clearTimeout(room.dcTimers[ws.seat]); room.dcTimers[ws.seat] = setTimeout(() => { if (rooms.get(room.code) === room && !room.players[ws.seat]) { clearTimeout(room.placementTimer); clearTimeout(room.finishTimer); rooms.delete(room.code); } }, RECONNECT_GRACE_MS); }
function removeMembership(ws) { const room = roomFor(ws); if (!room || room.players[ws.seat] !== ws) { ws.roomCode = null; ws.seat = null; return; } room.players[ws.seat] = null; clearTimeout(room.dcTimers[ws.seat]); if (!room.players[0] || !room.players[1]) { clearTimeout(room.waitingTimer); clearTimeout(room.placementTimer); clearTimeout(room.finishTimer); rooms.delete(room.code); } else { send(room.players[1 - ws.seat], "opponentDisconnected", {}); } ws.roomCode = null; ws.seat = null; }
...
wss.on("connection", (ws, req) => { const origin = req.headers.origin; if (origin && origin !== `http://${req.headers.host}` && origin !== `https://${req.headers.host}`) return ws.close(1008, "origin"); ws.clientIp = req.socket.remoteAddress || "unknown"; ws.on("message", (raw) => { let msg; try { msg = JSON.parse(raw); } catch { return sendError(ws, "消息格式无效。"); }
  ...
  if (msg.type === "join") { const room = rooms.get(String(msg.code || "")); if (!room || room.players[1]) return sendError(ws, "房间不存在或已满。"); removeMembership(ws); clearTimeout(room.waitingTimer); room.players[1] = ws; room.names[1] = cleanName(msg.playerName, "玩家 2"); room.tokens[1] = makeToken(); ws.roomCode = room.code; ws.seat = 1; room.state.placementDeadline = Date.now() + PLACEMENT_MS; room.players.forEach((p, seat) => send(p, "start", { code: room.code, gameId: "minesweeper-duel", seat, playerNames: room.names, phase: "PLACING", token: room.tokens[seat], placementDeadline: room.state.placementDeadline })); room.placementTimer = setTimeout(() => autoPlacement(room), PLACEMENT_MS); return; }
  ...
  if (msg.type === "restart") { const room = roomFor(ws); if (!room || room.state.phase !== "FINISHED") return sendError(ws, "当前不能再来一局。"); room.restartVotes ||= new Set(); room.restartVotes.add(ws.seat); if (room.restartVotes.size === 2) { room.state = newState(); room.state.placementDeadline = Date.now() + PLACEMENT_MS; room.restartVotes.clear(); clearTimeout(room.placementTimer); clearTimeout(room.finishTimer); room.placementTimer = setTimeout(() => autoPlacement(room), PLACEMENT_MS); broadcast(room, "restart", { phase: "PLACING", placementDeadline: room.state.placementDeadline }); } else send(ws, "restartPending", {}); return; }
  if (msg.type === "rejoin") { const room = rooms.get(String(msg.code || "")), seat = Number(msg.seat), token = msg.token; if (!room || ![0, 1].includes(seat) || typeof token !== "string" || !token || typeof room.tokens[seat] !== "string" || !room.tokens[seat] || token !== room.tokens[seat] || room.players[seat]) return sendError(ws, "重连失败，房间已失效。"); room.players[seat] = ws; ws.roomCode = room.code; ws.seat = seat; clearTimeout(room.dcTimers[seat]); send(ws, "rejoined", { code: room.code, gameId: "minesweeper-duel", seat, playerNames: room.names, state: publicState(room, seat) }); send(room.players[1 - seat], "opponentReconnected", {}); return; }
