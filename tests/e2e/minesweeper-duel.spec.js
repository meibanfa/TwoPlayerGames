...
test("mobile flag mode fits the board and toggles a flag", async ({ browser }) => {
  const context = await browser.newContext({ viewport: { width: 390, height: 844 }, serviceWorkers: "block" }); const a = await context.newPage();
  try { await create(a, "甲"); expect(await a.locator(".duel-board").evaluate((el) => el.getBoundingClientRect().right <= window.innerWidth)).toBe(true); await expect(a.locator(".flag-mode")).toBeVisible(); } finally { await context.close(); }
});
