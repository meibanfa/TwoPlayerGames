...
test("two browsers play a deterministic match and rematch", async ({ browser }) => {
  ...
  try {
    ...
    await expect(a.locator(".duel-message")).toContainText("你赢了"); await expect(a.locator(".mine-preview")).toHaveCount(14);
...
test("sweeping browser never receives opponent placement coordinates", async ({ browser }) => {
  ...
  const containsSecret = (value) => Array.isArray(value) ? value.some(containsSecret) || (value.length === secret.length && value.every((cell) => secret.includes(cell))) : value && typeof value === "object" ? Object.values(value).some(containsSecret) : false;
  try { await create(a, "甲"); const code = (await a.locator("#roomLabel").textContent()).match(/\d{4}/)[0]; await join(b, code, "乙"); await place(a, secret); await place(b, Array.from({ length: 15 }, (_, i) => i + 20)); await expect(b.locator(".duel-phase")).toHaveText("扫雷阶段"); const browserData = await b.evaluate(() => ({ html: document.documentElement.outerHTML, local: Object.values(localStorage), session: Object.values(sessionStorage) })); expect(frames.some(containsSecret)).toBe(false); expect(containsSecret(browserData)).toBe(false); } finally { await aContext.close(); await bContext.close(); }
});
